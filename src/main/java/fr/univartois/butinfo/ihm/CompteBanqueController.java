...
public class CompteBanqueController extends ApplicationBanque{
    ...
    private void initialize() {
        ...
        if (compteActuel != null) {
            historiqueTable.setItems(FXCollections.observableArrayList(compteActuel.getOperations()));
        ...
        try {
            if (compteActuel != null) {
                ...
                plafondVirementLabel.setText(String.format("%,.2f €",
                        (double) CompteBanque.MAX_VIREMENT - (double) compteActuel.getVirement()));
                nomTitulaireLabel.setText(compteActuel.getNomProprietaire());
            }
        } catch (Exception e) {
            showAlert(Alert.AlertType.ERROR, "Erreur", e.getMessage());
        }
    ...
    public void setCompteActuel(CompteBanque compte) {
        ...
        historiqueTable.setItems(FXCollections.observableArrayList(compte.getOperations()));
        historiqueTable.setItems(historique);
        updateInterface();
    }
